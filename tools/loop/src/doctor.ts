...
export async function runDoctor(target: string): Promise<DoctorReport> {
  ...
  if (auditRun.code !== 0 && !auditRun.stdout.trim()) {
    ...
    const j = parseJsonLoose(auditRun.stdout) as {
      score?: number;
      level?: string;
      assessment?: string;
  ...
  let syncLevel: string | null = null;
  let issueCount = 0;
  ...
  if (syncRun.code !== 0 && !syncRun.stdout.trim()) {
    ...
    const j = parseJsonLoose(syncRun.stdout) as {
      score?: number;
      level?: string;
      issues?: unknown[];
      suggestions?: string[];
  ...
  const byText = new Map<string, DoctorAction>();
  for (const a of actions) {
    const prev = byText.get(a.text);
    if (!prev || a.priority < prev.priority) byText.set(a.text, a);
  }
  const topActions = [...byText.values()].sort((a, b) => a.priority - b.priority).slice(0, 3);

  // Severity / exit code
  let severity: DoctorSeverity = 'healthy';
  let exitCode = 0;

  const blocked =
    (readyScore !== null && readyScore < 40) ||
    syncLevel === 'critical' ||
    (!stateName && !loopMd);

  const warning =
    (readyScore !== null && readyScore < 58) ||
    syncLevel === 'warning' ||
    issueCount > 0 ||
...
export function formatDoctorHuman(r: DoctorReport): string {
  ...
  const score =
    r.ready.score !== null ? `${r.ready.score}/100 ${r.ready.level ?? ''}`.trim() : 'n/a';
  lines.push(`Ready:   ${score}`);
  if (r.ready.assessment) lines.push(`         ${r.ready.assessment}`);
  const sync =
    r.sync.score !== null
  ...
  lines.push(
    `Files:   state=${r.files.state ?? 'missing'}  LOOP.md=${yn(r.files.loopMd)}  budget=${yn(r.files.budget)}  run-log=${yn(r.files.runLog)}  gate=${yn(r.files.gate)}`,
  );
